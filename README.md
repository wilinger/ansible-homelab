...
For my homelab, hosts are configured with [Vault SSH CA role](roles/vault-ssh-ca) to accept connections with signed certificates and authenticated with [client SSH task](roles/vault-ssh-ca/tasks/ssh-client.yml).  Refer to [k8s.yml](k8s.yml) playbook for example usage.
